...
@property (nonatomic, strong) RTCDataChannel *myDatachannel;
...
- (void)dataChannelDidChangeState:(RTCDataChannel *)dataChannel {    
    if ([dataChannel readyState] == RTCDataChannelStateOpen) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(webRTCPeer:didAddDataChannel:)]) {
...
- (void)sendDatachannelMessage:(NSString *)message {
    /*if (_myDatachannel) {
        ...
        NSLog(@"MY DATACHANNEL IS SENDING MESSAGE: %@", message);
        NSLog(@"DATACHANNEL STATE: %ld", [_myDatachannel readyState]);
        
        BOOL send = [_myDatachannel sendData:buffer];
        NSLog(@"IS SENT: %@", send?@YES:@NO);
    } else {
        NSLog(@"NO DATACHANNEL STORED");
    }*/
}
